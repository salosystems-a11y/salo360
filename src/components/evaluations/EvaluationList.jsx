          ...
          <Card className={`bg-card border-border card-hover overflow-hidden w-full ${isPendingList ? 'border-primary/50' : ''}`}>
            ...
            <CardContent className="space-y-2.5 text-xs">
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-medium">{getUserName(evaluation.evaluated_id)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t('evaluator')}:</span>
                <span className="text-foreground">{getUserName(evaluation.evaluator_id)}</span>
              </div>
              <div className="flex justify-between">
              ...
              {evaluation.scores && Object.keys(evaluation.scores).length > 0 && evaluation.status === 'completed' && (
               ...
               <div className="pt-2.5 flex space-x-2">
                {isPendingList && evaluation.evaluator_id === currentUser.id && evaluation.status === 'pending' ? (
                   <Button 
                    size="sm"
    ...
    const EvaluationList = ({ 
      ...
      const handleOpenResponse = (evaluation) => {
        if (evaluation.evaluator_id !== rest.currentUser.id || evaluation.status !== 'pending') {
      ...
      const handleSubmitResponse = async (evaluationId, scores, comments) => {
        const { error } = await supabase
          .from('evaluations')
          .update({ scores, comments, status: 'completed', completed_at: new Date().toISOString() })
          .eq('id', evaluationId);
